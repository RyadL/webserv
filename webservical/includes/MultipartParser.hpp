...
private:
    ...
    int primary_check_long_body(std::size_t, char*);
public:
    MultipartParser();
    MultipartParser(const char*, std::size_t, std::string);
    MultipartParser(FILE*, std::size_t, std::string);

    std::size_t find_boundary_start_short_body(std::size_t);
    std::size_t find_boundary_start_long_body(std::size_t);
    //int primary_check(std::size_t index);
    ...
    int further_validity_checks_long_body(std::size_t, char*);
