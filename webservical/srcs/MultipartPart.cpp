...
{
    std::cout << "MultipartPart default constructor" << std::endl;
}

MultipartPart::MultipartPart(const char* str)
:_header(str)
{
    std::cout << "MultipartPart constructor" << std::endl;
}

MultipartPart::~MultipartPart()
{
    std::cout << "MultipartPart default destructor" << std::endl;
}
...
void    MultipartPart::parse_header()
{
    ...
    while (i < _header.size())
    {
        ...
        std::cout << "FOUND0: " << found0 << " et FOUND1: " << found1 << std::endl;
        if (found1 != std::string::npos && found1 < found0)
        {
            i = found1 + 1;
            std::cout << _header.substr(i, 10) << std::endl;
        }
        ...
        i = found2 + 1;
        if (_header[i] == '\r')
            std::cout << "DAC0" << std::endl;
        if (_header[i] == '\n')
        {
            std::cout << "DAC1" << std::endl;
            i++;
            std::cout << _header.c_str() + i << std::endl;
        }
        if (found0 == std::string::npos)
            header_end = false;

        std::cout << "FIELD_NAME: " << field_name << " FIELD_VALUE: " << field_value << std::endl;
        /*
        i = found2 + 1;   
        if (_header[found2] == '\r')
            found2++;
