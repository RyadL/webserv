...
class socket_class 
{
    ...
    int get_header ( std::map<int, client_buffer*>::iterator it )
    {
        ...
        //std::cout << "\x1B[37mit->second->buffer ->\033[0m" << it->second->buffer << "\x1B[37m<- END\033[0m" << std::endl;
    ...
    int get_chunk_body ( std::map<int, client_buffer*>::iterator it )
    {
        ...
        /*for (int i = 0; i < ret; i++)
        {
            ...
            if (it->second->bytes_inside_chunk_body > 1)
            {
                ...
                { 
                    //std::cout << "On a trouve un separateur \r\n" << it->second->bytes_inside_chunk_body << std::endl;
                    //std::cout << "Bytes inside chunk body = " << it->second->bytes_inside_chunk_body << " total = " << it->second->total << "to write = " << it->second->chunktowrite << std::endl;
                    if (strncmp(it->second->short_body + it->second->bytes_inside_chunk_body - 3, "\n", 1) && it->second->am_i_inside_chunk == false)
                    {
                        std::cout << "Ce separateur nest pas un X4 de fin de chunk" << it->second->bytes_inside_chunk_body << std::endl;
                        std::cout << "Bytes inside chunk body = " << it->second->bytes_inside_chunk_body << " total = " << it->second->total << " to write = " << it->second->chunktowrite << std::endl;
                        if (ft_atoi_base(it->second->buffer, 16) && it->second->am_i_inside_chunk == false)
                        {
                            std::cout << "On est pas dans un chunk " << it->second->bytes_inside_chunk_body << std::endl;
                            std::cout << "Bytes inside chunk body = " << it->second->bytes_inside_chunk_body << " total = " << it->second->total << " to write = " << std::endl;
                            number = ft_atoi_base(it->second->buffer, 16);

                            memset(it->second->short_body + it->second->bytes_inside_chunk_body - ( i + 1 ) , 0, i + 1 );
                            it->second->bytes_inside_chunk_body = it->second->bytes_inside_chunk_body - i - 1;
                            it->second->chunktowrite = number;
                            it->second->am_i_inside_chunk = true;

                            if (it->second->chunktowrite == 0)
                                return 1;
                            continue;
                        }
                    }
                    ...
                    {
                        //std::cout << "Premier separateur  dun chunk "<< it->second->bytes_inside_chunk_body << std::endl;
                        //std::cout << "Bytes inside chunk body = " << it->second->bytes_inside_chunk_body << " total = " << it->second->total << " to write = " << it->second->chunktowrite << std::endl;
                        //std::cout << "Ou recommencer le atoi ?? " << it->second->utils << " donne => " << (it->second->short_body[it->second->utils]) << std::endl;
                        number = ft_atoi_base(it->second->short_body + it->second->utils, 16);
                        if (number == 0)
                        {
                            //std::cout << "Le premier separateur donne un atoi de 0; regardons la string : " << it->second->bytes_inside_chunk_body <<std::endl;
                            //std::cout << "Bytes inside chunk body = " << it->second->bytes_inside_chunk_body << " total = " << it->second->total << " to write = " << it->second->chunktowrite << std::endl;

                            //for(int i = it->second->bytes_inside_chunk_body - 8; i < it->second->bytes_inside_chunk_body + 15; i++)
                                //std::cout << "->" << it->second->short_body[i] << "<-" << std::endl; 
                            //std::cout << std::endl << "END CHUNK CLASSIQUE 1" << std::endl << std::endl;
                            ...
                            //std::cout << "Un paquet de fin a ete recu" << std::endl;
                            it->second->state_suspended = false;
                            return 1;
                        }
                        it->second->chunktowrite = number;
                        ...
                        it->second->copy = false;
                        //std::cout << "Bytes inside chunk body = " << it->second->bytes_inside_chunk_body << " total = " << it->second->total << " to write = " << it->second->chunktowrite << std::endl;
                        //std::cout << it->second->short_body << std::endl;
                    }
                }
            }
        }*/
    ...
    int get_short_body ( std::map<int, client_buffer*>::iterator it )
    {
        ...
        if (!it->second->state_suspended)
        {
            ...
            it->second->bytes_inside_body = it->second->body_size;
